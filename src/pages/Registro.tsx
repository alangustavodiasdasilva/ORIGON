...
export default function Registro() {
    ...
    const loadNextForReview = async (item?: PendingConfirmation) => {
        ...
        if (reviewItem) {
            ...
            if (mediaRow) {
                ...
                const avg = rows.reduce((acc, curr) => ({
                    numero: 'M',
                    hvi: curr.hvi,
                    data_analise: curr.data_analise,
                    hora_analise: curr.hora_analise,
                    mic: acc.mic + (curr.mic / rows.length),
                    len: acc.len + (curr.len / rows.length),
                    unf: acc.unf + (curr.unf / rows.length),
                    str: acc.str + (curr.str / rows.length),
                    rd: acc.rd + (curr.rd / rows.length),
                    b: acc.b + (curr.b / rows.length),
                }), { mic: 0, len: 0, unf: 0, str: 0, rd: 0, b: 0 } as any);
                rows = [avg];
            }
    ...
    const handleSaveSelectedSamples = async () => {
        if (!loteId || !pendingReview || selectedRows.size === 0 || isSaving) return;

        setIsSaving(true);
        try {
            console.log("Iniciando salvamento de amostras...", { rows: editingRows.length, selected: selectedRows.size });
            ...
            const dataList = rowsToSave.map((row) => {
                ...
                if (!currentNextId) {
                    currentNextId = (samples.length + existingIds.size).toString().padStart(2, '0');
                }
                    ...
                    <div className="absolute top-0 left-0 right-0 h-16 bg-white border-b border-black flex items-center justify-between px-8 z-20">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <Button 
                                ...
                                disabled={isSaving}
                                className="h-10 rounded-none bg-black text-white hover:bg-neutral-800 text-[10px] uppercase tracking-widest px-8 font-bold shadow-lg transition-all hover:scale-105 disabled:opacity-50 disabled:scale-100"
                            >
                                {isSaving ? (
                                    <>
