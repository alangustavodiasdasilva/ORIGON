...
export default function MonitoramentoOS() {
    ...
    const processOSData = useCallback((rawData: OSItem[]) => {
        const mappedData = rawData.map(os => {
            ...
            const isInvalidText = (str?: string) => {
                if (!str) return false;
    ...
    return (
        <div className="max-w-[1600px] mx-auto py-12 px-6 text-black pb-32 min-h-screen font-sans">
            ...
            <div
                className={cn(
                    "fixed top-0 left-0 right-0 z-[9999] h-[2px] pointer-events-none transition-opacity duration-400",
                    isLoading ? "opacity-100" : "opacity-0"
                )}
            >
                <div
                    className={cn(
                        "h-full bg-gradient-to-r from-transparent via-blue-500 to-transparent bg-[length:200%_100%]",
                        isLoading && "animate-progress-infinite"
                    )}
                />
            </div>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-10 mb-12 animate-in fade-in slide-in-from-top duration-700">
                ...
                <div className="flex flex-col md:flex-row flex-wrap gap-4 items-center">
                    ...
                    <div className="flex items-center gap-1 bg-neutral-100 p-1.5 rounded-xl border border-neutral-200/50">
                        ...
                        >
                            <RefreshCw
                                className={cn("h-3.5 w-3.5 mr-2 transition-transform", isLoading && "animate-spin")}
                            />
                            <span className={cn("transition-opacity duration-300", isLoading ? "opacity-50" : "opacity-100")}>
                            ...
                            {labs.map((lab) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            "flex items-center gap-2 px-4 py-2 rounded-xl border-2 transition-all text-[10px] font-black uppercase tracking-widest shrink-0 relative overflow-hidden",
                                    ...
                                    >
                                        ...
                                        <div className="relative z-10 flex items-center gap-2">
                                            <svg width="6" height="6" viewBox="0 0 6 6">
                                                <circle cx="3" cy="3" r="3" fill={isActive ? 'white' : color} />
                                            </svg>
                                            {lab.nome}
                                        </div>
                                    </button>
