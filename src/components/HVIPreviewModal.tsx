        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
            ...
            <div className="w-full max-w-7xl max-h-[90vh] bg-white flex flex-col shadow-2xl border border-black">
                ...
                <div className="flex-1 overflow-hidden bg-neutral-50 flex flex-col lg:flex-row">
                    {/* LEFT COLUMN: Tables */}
                    <div className="w-full lg:w-7/12 p-4 sm:p-6 overflow-y-auto border-b lg:border-b-0 lg:border-r border-neutral-200">
                        {/* TOP COMPARATIVE TABLE */}
                    <div className="bg-white border border-neutral-200 shadow-sm mb-4">
                        <div className="p-3 bg-neutral-50/50 border-b border-neutral-200 flex justify-between items-center">
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="bg-white border-b border-neutral-200">
                                        <th className="px-4 py-2 text-left font-bold text-xs uppercase tracking-wider text-neutral-400 w-32 border-r border-neutral-200">Fonte</th>
                                        <th className="px-2 py-2 text-center font-bold text-xs uppercase tracking-wider text-neutral-500 w-1/6">MIC</th>
                                        <th className="px-2 py-2 text-center font-bold text-xs uppercase tracking-wider text-neutral-500 w-1/6">LEN</th>
                                        <th className="px-2 py-2 text-center font-bold text-xs uppercase tracking-wider text-neutral-500 w-1/6">UNF</th>
                                        <th className="px-2 py-2 text-center font-bold text-xs uppercase tracking-wider text-neutral-500 w-1/6">STR</th>
                                        <th className="px-2 py-2 text-center font-bold text-xs uppercase tracking-wider text-neutral-500 w-1/6">RD</th>
                                        <th className="px-2 py-2 text-center font-bold text-xs uppercase tracking-wider text-neutral-500 w-1/6">+B</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {/* ROW A - Original */}
                                    <tr className="border-b border-neutral-100 hover:bg-blue-50/30 transition-colors">
                                        <td className="px-4 py-2 border-r border-neutral-200 bg-blue-50/20">
                                        ...
                                        <td className="px-2 py-2 text-center font-bold text-sm text-blue-600">{typeof originalSample.mic === 'number' ? originalSample.mic.toFixed(2) : (originalSample.mic || '-')}</td>
                                        <td className="px-2 py-2 text-center font-bold text-sm text-blue-600">{typeof originalSample.len === 'number' ? originalSample.len.toFixed(2) : (originalSample.len || '-')}</td>
                                        <td className="px-2 py-2 text-center font-bold text-sm text-blue-600">{typeof originalSample.unf === 'number' ? originalSample.unf.toFixed(1) : (originalSample.unf || '-')}</td>
                                        <td className="px-2 py-2 text-center font-bold text-sm text-blue-600">{typeof originalSample.str === 'number' ? originalSample.str.toFixed(1) : (originalSample.str || '-')}</td>
                                        <td className="px-2 py-2 text-center font-bold text-sm text-blue-600">{typeof originalSample.rd === 'number' ? originalSample.rd.toFixed(1) : (originalSample.rd || '-')}</td>
                                        <td className="px-2 py-2 text-center font-bold text-sm text-blue-600">{typeof originalSample.b === 'number' ? originalSample.b.toFixed(1) : (originalSample.b || '-')}</td>
                                    </tr>
                                    {/* ROW B - Generated */}
                                    <tr className="hover:bg-green-50/30 transition-colors">
                                        <td className="px-4 py-2 border-r border-neutral-200 bg-green-50/20">
                                            <div className="flex items-center gap-2">
                                        ...
                                        <td className="px-2 py-2 text-center font-bold text-sm text-green-600">{generatedValues.mic.toFixed(2)}</td>
                                        <td className="px-2 py-2 text-center font-bold text-sm text-green-600">{generatedValues.len.toFixed(2)}</td>
                                        <td className="px-2 py-2 text-center font-bold text-sm text-green-600">{generatedValues.unf.toFixed(1)}</td>
                                        <td className="px-2 py-2 text-center font-bold text-sm text-green-600">{generatedValues.str.toFixed(1)}</td>
                                        <td className="px-2 py-2 text-center font-bold text-sm text-green-600">{generatedValues.rd.toFixed(1)}</td>
                                        <td className="px-2 py-2 text-center font-bold text-sm text-green-600">{generatedValues.b.toFixed(1)}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    {balancedReadings && balancedReadings.mic && (
                        <div className="bg-white border border-neutral-200 p-4 mb-2">
                            ...
                            <div className="overflow-x-auto border border-neutral-200">
                                <table className="w-full text-sm">
                                    <thead className="bg-neutral-50 text-xs uppercase font-bold text-neutral-500 border-b border-neutral-200">
                                        <tr>
                                            <th className="px-2 py-1.5 text-center border-r border-neutral-200">#</th>
                                            <th className="px-2 py-1.5 text-center border-r border-neutral-200">MIC</th>
                                            <th className="px-2 py-1.5 text-center border-r border-neutral-200">LEN</th>
                                            <th className="px-2 py-1.5 text-center border-r border-neutral-200">UNF</th>
                                            <th className="px-2 py-1.5 text-center border-r border-neutral-200">STR</th>
                                            <th className="px-2 py-1.5 text-center border-r border-neutral-200">RD</th>
                                    ...
                                    <tbody className="divide-y divide-neutral-100">
                                        {editableReadings.mic?.map((_, idx) => (
                                            <tr key={idx} className="hover:bg-blue-50/30 transition-colors">
                                                <td className="px-2 py-1 text-center font-bold text-neutral-400 border-r border-neutral-200 bg-neutral-50">
                                                    {idx + 1}
                                                </td>
                                        ...
                                        {editableReadings.mic && editableReadings.mic.length > 0 && (
                                            <tr className="bg-neutral-100 font-bold border-t-2 border-neutral-300">
                                                <td className="px-2 py-2 text-center text-[10px] uppercase tracking-widest text-neutral-600 border-r border-neutral-300">
                                                    MÉDIA
                                                </td>
                                                {['mic', 'len', 'unf', 'str', 'rd', 'b'].map(field => {
                                                    ...
                                                    const maxDecimals = (field === 'mic' || field === 'len') ? 2 : 1;
                                                    return (
                                                        <td key={field} className="px-2 py-2 text-center text-blue-700 border-r border-neutral-300 last:border-0">
