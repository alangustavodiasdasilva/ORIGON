...
export default function MonitoramentoOS() {
    ...
    // Lista Filtrada Base para todas as agrega├º├Áes
    ...
    const revisorStats = React.useMemo(() => {
        ...
        filteredOS.forEach((os: OSItem) => {
            const rev = os.revisor || 'N├úo Informado';
    ...
    const clienteStats = React.useMemo(() => {
        ...
        filteredOS.forEach((os: OSItem) => {
            const cli = os.cliente || 'N├úo Informado';
    ...
    // Agrega├º├úo para Gr├ífico de Produtividade Di├íria por Revisor
    ...
    // Agrega├º├úo para Gr├ífico de Volume Di├írio por Cliente
    ...
    // Agrega├º├úo para Tabela Pivot de Saldo de An├ílises Di├írio (N├úo Finalizadas)
    const saldoDiarioPivotStats = React.useMemo(() => {
        ...
        // Utiliza filteredOS para que a busca funcione nesta aba tamb├®m
        filteredOS.forEach((os: OSItem) => {
            // S├ô entra se data_finalizacao estiver em BRANCO
        ...
        try {
            ...
            const top5 = data
                .reduce((acc: any, curr: any) => {
                    const cli = curr.cliente || 'N├úo Informado';
            ...
            setSelectedReviewers([...revs, 'Volume Produzido (An├ílise)', 'Total Revisado (Analistas)']);
        } catch (error) {
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!labId) {
            alert("Erro: Nenhum laborat├│rio selecionado. Por favor, selecione um laborat├│rio no menu superior.");
            return;
        }
        ...
        try {
            ...
            if (totalRecords === 0) {
                alert("Nenhum dado v├ílido encontrado no arquivo.");
                return;
            }
            ...
            if (msg.includes("column") && msg.includes("horas")) {
                alert("Erro: A coluna 'horas' n├úo existe no banco de dados. Por favor, execute o script SQL atualizado.");
            } else {
    ...
    const handleClearData = async () => {
        ...
        if (!window.confirm("ATEN├ç├âO: Tem certeza que deseja apagar TODOS os dados de monitoramento?\n\nEsta a├º├úo n├úo pode ser desfeita.")) {
            return;
        }
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            pdf.text("ORIGO INTELLIGENCE - RELAT├ôRIO DE SALDO DI├üRIO", 10, 12);

            pdf.setFontSize(8);
            pdf.setFont("helvetica", "normal");
            pdf.text(`LABORAT├ôRIO: ${currentLab?.nome || 'N├âO IDENTIFICADO'}`, 10, 17);
            pdf.text(`DATA EMISS├âO: ${format(new Date(), 'dd/MM/yyyy HH:mm')}`, pdfWidth - 60, 17);
            ...
            pdf.text("An├ílise baseada no monitoramento em tempo real de O.S. pendentes.", 10, footerY);
            pdf.text(`P├ígina 1 de 1`, pdfWidth - 25, footerY);
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white border border-neutral-200 rounded-xl p-4 shadow-sm animate-fade-in text-black">
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-400">Filtrar Gr├ífico (Revisores & Totais)</h3>
                            <div className="flex gap-4">
                                <button
                                    onClick={() => setSelectedReviewers([...revisorDailyStats.keys, 'Volume Produzido (An├ílise)', 'Total Revisado (Analistas)'])}
                        ...
                        <div className="flex flex-wrap gap-2">
                            {/* Static KPI Lines */}
                            <button
                                onClick={() => toggleReviewerSelection('Volume Produzido (An├ílise)')}
                                className={cn(
                                    "px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all border flex items-center gap-2",
                                    selectedReviewers.includes('Volume Produzido (An├ílise)')
                    ...
                    <div className="bg-white border border-neutral-200 rounded-xl p-6 shadow-sm text-black">
                        <h3 className="font-bold text-sm uppercase tracking-wide mb-6">Evolu├º├úo Di├íria (An├ílise Opera├º├úo vs Revis├úo Analistas)</h3>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={revisorDailyStats.data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                    ...
                                    {selectedReviewers.includes('Volume Produzido (An├ílise)') && (
                                        <Line
                                            type="monotone"
                                            name="Volume Produzido (An├ílise)"
                ...
                <div className="space-y-6 animate-fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-neutral-900 text-white p-6 rounded-xl border border-neutral-800 shadow-lg">
                            ...
                            <div className="text-[9px] uppercase font-mono mt-1 text-neutral-500">Amostras sem finaliza├º├úo</div>
                        </div>
                        <div className={`p-6 rounded-xl border shadow-sm ${saldoDiarioPivotStats.criticalCount > 0 ? 'bg-red-50 border-red-200' : saldoDiarioPivotStats.sortedDelayed.length > 0 ? 'bg-amber-50 border-amber-200' : 'bg-white border-neutral-200'}`}>
                            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-400 block mb-2">Estado Cr├¡tico</span>
                        ...
                        <div className={`p-6 rounded-xl border shadow-sm ${saldoDiarioPivotStats.avgDelay > 24 ? 'bg-neutral-50 border-neutral-200' : 'bg-white border-neutral-200'}`}>
                            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-400 block mb-2">M├®dia de Espera</span>
                            ...
                            <div className="text-[9px] uppercase font-mono mt-1 text-neutral-400">Tempo m├®dio de an├ílise</div>
                        </div>
                    </div>
                    ...
                    <div id="matrix-capture-container" ref={matrixTableRef} className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
                        <div className="p-4 border-b border-neutral-100 bg-neutral-900 text-white flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="font-bold text-sm uppercase tracking-wide">Saldo de An├ílises Di├írio por Produtor</h3>
                            </div>
                            <div className="flex items-center gap-4">
                                <button
                                ...
                                >
                                    ...
                                    {isGeneratingPDF ? "GERANDO..." : "RELAT├ôRIO PDF"}
                                </button>
                        ...
                        <div className="bg-white border border-red-100 rounded-xl overflow-hidden shadow-md">
                            <div className="p-4 border-b border-red-50 bg-white flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-sm uppercase tracking-wide text-red-700">Amostras com Atraso Cr├¡tico</h3>
                                        <div className="text-[10px] text-red-400 font-medium uppercase font-mono tracking-wider">Aten├º├úo Necess├íria Imediata</div>
                                    </div>
                                </div>
            ...
            {activeTab === 'clientes' && (
                <div className="space-y-6">
                    ...
                    <div className="bg-white border border-neutral-200 rounded-xl p-4 shadow-sm">
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-400">Filtrar Gr├ífico (Top Clientes)</h3>
                            <button
                    ...
                    <div className="bg-white border border-neutral-200 rounded-xl p-6 shadow-sm">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="font-bold text-sm uppercase tracking-wide">Evolu├º├úo Di├íria (Volume por Cliente)</h3>
                            ...
                            <table className="w-full text-sm text-left">
                                <thead className="bg-neutral-50 text-[10px] uppercase font-bold text-neutral-500 tracking-wider font-mono">
                                    <tr>
                                        ...
                                        <th className="p-4">Produtor / Cliente</th>
                                        <th className="p-4 text-center">Tempo M├®dio (H)</th>
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                        ...
                        <div className="bg-white border p-6 rounded-xl shadow-sm border-amber-200">
                            <div className="flex justify-between items-start mb-2">
                                <span className="text-[10px] font-bold uppercase tracking-widest text-amber-500">Saldo de An├ílise</span>
                    ...
                    <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
                        <div className="p-4 border-b border-neutral-100 flex justify-between items-center bg-neutral-50">
                            <h3 className="font-bold text-sm uppercase tracking-wide">Hist├│rico Simplificado</h3>
